...
export const RIGHT_TRIANGLE_SVG = `<svg xmlns="http://www.w3.org/2000/svg" width="600" height="380" viewBox="0 0 600 380">
...
export function generateRightTriangleDataUrl(): string {
  const base64Svg = Buffer.from(RIGHT_TRIANGLE_SVG).toString('base64');
...
function cleanText(input: string): string {
  ...
  // Converte comandos e expressões LaTeX para texto claro e legível em português
  text = cleanLatexMath(text);
  text = text.replace(/Resposta:\s*___/gi, ''); // remove linhas soltas "Resposta: ___" duplicadas
...
function parseAtividade(html: string): { titulo: string; linhas: string[] } {
  ...
  const linhas = rawLinhas.filter((l) => {
    if (isAsciiArtLine(l)) return false;
    // Remove marcadores crus de figura [Figura: ...]
    if (/^\[Figura:.*\]$/i.test(l.trim())) return false;
    return true;
  });
...
export async function gerarDOCX(atividade: AtividadeData): Promise<Buffer> {
  ...
  // Inserção de Figuras/Diagramas no DOCX
  const ehGeometria =
  ...
  const listaImagensDocx: string[] = atividade.imagens ? [...atividade.imagens] : [];
  if (ehGeometria && listaImagensDocx.length === 0) {
